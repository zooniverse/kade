...
RSpec.describe 'Admin resource pages', type: :request do
  ...
  describe 'context manual triggers' do
    it 'queues a prediction job trigger from the context page' do
      job_id = 'prediction-jid-123'
      allow(PredictionManifestExportJob).to receive(:perform_async).with(context.id).and_return(job_id)
      ...
      expect(response.body).to include("Prediction job with id #{job_id} triggered")
    ...
    it 'queues a training job trigger from the context page' do
      job_id = 'training-jid-123'
      allow(RetrainZoobotJob).to receive(:perform_async).with(context.id).and_return(job_id)
      ...
      expect(response.body).to include("Training job with id #{job_id} triggered")
